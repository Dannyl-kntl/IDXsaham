...
class IDXDataFetcher:
    def __init__(self):
        ...
        # Buat session untuk reuse koneksi (lebih efisien)
    ...
    def _fetch_from_yfinance(self, ticker):
        ...
        try:
            # Gunakan session yang sama untuk menghindari pembukaan koneksi baru
            stock = yf.Ticker(f"{ticker}.JK", session=self.session)
            info = stock.info
            
            # HAPUS pemanggilan stock.history() karena tidak digunakan (growth selalu 0)
            # hist = stock.history(period="5y")  <-- DIHAPUS
            ...
            return {
                ...
                # Growth di-set 0 karena kita tidak punya data riil
                "revenue_growth_5y": 0,
                "profit_growth_5y": 0,
    ...
    # Fungsi _calculate_growth dihapus karena tidak digunakan
