...
class StockScreener:
    ...
    def _calculate_score(self, data):
        """Hitung skor berdasarkan 10 kriteria"""
        score = 0
        max_score = 100
        ...
        if mcap > 100000000000000:
            mcap_score = 5
        elif mcap > 10000000000000:
            mcap_score = 3
        else:
            mcap_score = 1
        score += mcap_score
    ...
    def format_results(self, results):
        """Format hasil screening untuk Telegram"""
        if not results:
    ...
    def get_detail(self, ticker):
        """Dapatkan detail lengkap 1 saham"""
        data = self.fetcher.get_stock_info(ticker)
        if not data:
...
"""
    ...
    def _calculate_moat(self, data):
        """Hitung rating competitive moat"""
        score = 0
    ...
    def _calculate_risk(self, data):
        """Hitung rating risiko 1-10 dengan alasan"""
        reasons = []
        risk = 0
