# ===== SUPER SCREENER - FINAL (PRIORITAS + TIDAK TERPOTONG) =====
...
# ========== KONFIGURASI ==========
TOKEN = "8815512475:AAG9sdH2v8sbNPJ2GK6HlK-638szn1n32wA"
CHAT_ID = "8467853860"
# =================================
...
def kirim_pesan(pesan):
    """Kirim pesan ke Telegram (tanpa parse_mode, auto split)"""
    try:
        max_len = 4000
        if len(pesan) > max_len:
            print(f"⚠️ Pesan panjang ({len(pesan)} karakter). Dipotong menjadi beberapa bagian...")
...
def analisis_super(kode, df):
    try:
        ...
        # === PRIORITY SCORE (KEMUNGKINAN GAP UP) ===
        ...
        if 50 < rsi < 65:
            ...
            priority_score += 5
        if harga < df['MA50'].iloc[-1] and not pd.isna(df['MA50'].iloc[-1]):
            priority_score -= 5
...
# === SORTIR BERDASARKAN PRIORITY SCORE (TERTINGGI) ===
semua_hasil.sort(key=lambda x: x['priority_score'], reverse=True)
potensial = [h for h in semua_hasil if h['priority_score'] >= 70]
pantauan = [h for h in semua_hasil if 50 <= h['priority_score'] < 70]

print(f"✅ Berhasil menganalisis {len(semua_hasil)} saham.")
print(f"🔥 Potensial (Priority >= 70): {len(potensial)}")

# === PESAN TELEGRAM (HANYA 5 SAHAM PRIORITAS + 3 PANTAUAN) ===
pesan = f"🌟 SUPER SCREENER (PRIORITAS GAP UP)\n"
pesan += f"📅 {waktu_str} WITA\n"
pesan += f"📌 Total: {len(semua_hasil)} | 🔥 Prioritas: {len(potensial)}\n"
pesan += "=" * 30 + "\n\n"

if potensial:
    pesan += "🔥 SAHAM PRIORITAS TINGGI (Paling Berpotensi Gap Up)\n\n"
    for h in potensial[:5]:
        pesan += f"🏢 {h['kode']} - {h['nama']}\n"
        pesan += f"💰 Rp{h['harga']:.0f} | 📈 {h['perubahan']:+.2f}% | ⭐ {h['priority_score']:.0f}\n"
        pesan += f"   T:{h['skor_teknikal']}/50 | M:{h['skor_money']}/50\n"
    ...
    # Saham potensial sisanya (jika >5)
    if len(potensial) > 5:
        pesan += f"📊 + {len(potensial)-5} saham potensial lainnya (cek daftar lengkap)\n\n"
else:
    pesan += "⏳ Belum ada saham dengan prioritas tinggi hari ini.\n\n"

if pantauan:
    pesan += "📊 PANTAUAN (Priority 50-69)\n"
    for h in pantauan[:3]:
        pesan += f"🔹 {h['kode']} | {h['priority_score']:.0f} | RSI:{h['rsi']} | Rp{h['harga']:.0f}\n"
