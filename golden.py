...
# BATCH SIZE DIKECILKAN JADI 15 (Biar lebih cepat)
batch_size = 15
batches = list(chunk_list(LIST_SAHAM, batch_size))
print(f"📥 Total {len(LIST_SAHAM)} saham, dipecah menjadi {len(batches)} batch (@ {batch_size} per batch).")
...
for batch in batches:
    ...
    try:
        # Timeout 20 detik per batch (lebih pendek)
