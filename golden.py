# ===== GOLDEN SCANNER - REVISI (DOWNLOAD MASSAL + WITA) =====
# Strategi: Buy sore (15:30 WITA), sell pagi (gap up)
...
import pytz  # Untuk timezone WITA
import warnings
...
# ========== DAFTAR SAHAM (FLAT LIST, BUKAN TUPLE) ==========
# Saya perbaiki: Hapus tanda kurung () di luar, jadi list datar
LIST_SAHAM = [
...
def analisis_golden(kode, df):
    """Analisis dari dataframe yang sudah di-download massal"""
    try:
...
waktu_str = waktu_sekarang.strftime('%d-%m-%Y %H:%M')

print(f"🚀 GOLDEN SCANNER (WITA) - {waktu_str}")
print(f"📥 Mendownload data {len(LIST_SAHAM)} saham sekaligus...")
...
print(f"✅ Berhasil memproses {success_count} saham dari {len(LIST_SAHAM)}")
