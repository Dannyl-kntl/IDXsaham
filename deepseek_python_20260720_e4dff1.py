# ===== SUPER SCREENER (TEKNIKAL + MONEY FLOW) - GOLDEN.PY FINAL FIX =====
# Perbaikan: Error 400, entry variable, NaN handling
...
def kirim_pesan(pesan):
    """Kirim pesan ke Telegram tanpa format HTML (teks polos)"""
    try:
        url = f"https://api.telegram.org/bot{TOKEN}/sendMessage"
        # Tidak pakai parse_mode, kirim teks polos
        params = {
            'chat_id': CHAT_ID,
            'text': pesan
        }
...
def detect_support_resistance(df):
    ...
    if not is_breakout and current_close >= resistance:
        is_breakout = True
        breakout_strength = (current_close / support - 1) * 100 if support > 0 else 0
    else:
...
def analisis_super(kode, df):
    try:
        ...
        # === PERBAIKAN: entry, sl, tp, signal, rekom SELALU ADA ===
        if skor_total >= 70:
            entry = harga
...
# === PESAN TELEGRAM (TEKS POLOS, TANPA HTML) ===
...
if kirim_pesan(pesan):
    ...
    print("❌ Gagal mengirim pesan. Periksa Token dan Chat ID.")